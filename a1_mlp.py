...
def xavier_init(layer_sizes, rand_seed = 42):
    ...
    for i in range(1, L+1):
         F_in = layer_sizes[i-1]
         F_out = layer_sizes[i]

         sigma = np.sqrt(2 / (F_in + F_out))
         weights[i] = np.random.normal(0, sigma, (F_in, F_out))
         biases[i] = np.zeros((F_out, 1))
...
def forward_linear(A_prev, W, b, act_function):
    ...
    if act_function == 'sigmoid':
        ...
        raise ValueError(f"Invalid act_function: choose 'sigmoid' or 'tanh")
...
def backprop(y_hat, y_true, act_functions, caches):
    ...
    dA_prev = backprop_cost_layer(y_hat, y_true)
    ...
    for l in reversed(range(1, L+1)):
        ...
        dA_prev, dW, db = backprop_linear_layer(dA_prev, cache, act_functions[l])
